...
namespace cadeMedicoApi.Data
{
    public class Repository : IRepository
    {
        ...
        public async Task<MedicoModel[]> GetAllMedicoModelAsync(bool includeMedico = false)
        {
            ...
            if(includeMedico){
                query = query.Include(me => me.MedicoCidade).ThenInclude(mc => mc.Cidade)
                             .Include(em => em.MedicoEspecialidade).ThenInclude(e => e.Especialidade);
            }
        ...
        {
            IQueryable<MedicoModel> query = _context.Medicos;
            if (includeEspecialidade){query = query.Include(me => me.MedicoEspecialidade)
                                                   .ThenInclude(e => e.Especialidade);}
            query = query.AsNoTracking().OrderBy(medico => medico.Id).Where(medico => medico.MedicoEspecialidade.Any(me => me.EspecialidadeId
            == EspecialidadeId));
        ...
        public async Task<MedicoModel> GetMedicoModelById(int medicoId, bool includeCidade)
        {
            IQueryable<MedicoModel> query = _context.Medicos;
            if(includeCidade){
                query = query.AsNoTracking().Include(c => c.MedicoCidade).ThenInclude(mc => mc.Cidade)
                                            .Include(em => em.MedicoEspecialidade).ThenInclude(e => e.Especialidade);
            }
        ...
        public async Task<CidadeModel> GetCidadeModelById(int cidadeId, bool includeMedico)
        {
            IQueryable<CidadeModel> query = _context.Cidades;
            if (includeMedico){
                query = query.Include(mc => mc.MedicoCidade).ThenInclude(m => m.Medico).ThenInclude(me => me.MedicoEspecialidade).ThenInclude(e => e.Especialidade);
            }
